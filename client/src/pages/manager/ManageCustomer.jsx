...
import { apiGetCustomers, apiGetCustomersAdmin } from "~/apis/user"
...
const ManageCustomner = () => {
  ...
  const fetchContractCustomers = async (params) => {
    ...
    if (current.rroles.some((el) => el.roleCode === "ADMIN")) {
        response = await apiGetCustomersAdmin({
          limit: import.meta.env.VITE_LIMIT_CUSTOMER,
          ...params,
        });
    } else {
        response = await apiGetCustomers({
          limit: import.meta.env.VITE_LIMIT_CUSTOMER,
          ...params,
        });
    }
                  ...
                  <span className="flex items-center justify-center gap-4">
                    ...
                    <span
                        ...
                        onClick={current.rroles.some((role) => role.roleCode === "ADMIN") 
                          ? undefined 
                          : () => setModal(true, <UpdateUserByManager user={el} />)
                        }
                        className={`text-blue-600 hover:underline cursor-pointer text-center ${

...
import { apiGetPosts, apiRemovePost, apiGetAdminPosts } from "~/apis/post"
    ...
    <div className="w-full h-full">
      <div className="flex justify-between py-4 lg:border-b px-4 items-center">
        <h1 className="text-3xl font-bold">Quản lý tin đăng</h1>
          {current.rroles.some((el) => el.roleCode === "ADMIN") ? (
            <span
              className="py-2 px-4 bg-blue-600 text-white flex justify-center items-center rounded-md opacity-50 cursor-not-allowed"
            >
              Tạo mới
            </span>
          ) : (
              ...
              {posts?.rows?.map((el) => (
                <tr key={el.id}>
                  ...
                  <span className="flex items-center gap-4 justify-center">
                    <span
                      onClick={current.rroles.some((el) => el.roleCode === "ADMIN") ? undefined : () =>
                        navigate(`/${pathname.manager.LAYOUT}/${pathname.manager.UPDATE_POST}/${el.id}`)
                      }
                      className={`cursor-pointer hover:text-blue-600 ${current.rroles.some((el) => el.roleCode === "ADMIN") ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                      <FiEdit size={18} />
                    </span>
                    <span
                      onClick={current.rroles.some((el) => el.roleCode === "ADMIN") ? undefined : () => handleRemove(el.id)}
                      className={`cursor-pointer hover:text-blue-600 ${current.rroles.some((el) => el.roleCode === "ADMIN") ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
